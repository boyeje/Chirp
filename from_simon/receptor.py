	...
	while True:
		if sema>2*len(chirp_data_int):
			...
			if tiers2>tiers1*0.85 and tiers2<tiers1 and tiers3>tiers1*0.50 and tiers3<tiers1*0.65:
				...
				print "FREQ OK"
			print max(conv_env)
			if  max(conv_env)>0.5:  #freq_test or and (sum(conv_env[peak_env-5:peak_env+5]>0.5*max(conv_env))<2)
...
if __name__=='__main__':
	...
	global call_chirp,call_flag,call_input,sema,nbrCall,PEAK,PEAK_FLAG,AUTOCAL
	AUTOCAL=14230 #a definir a l'aide de la fonction d'autocalibration
	PEAK=1000000000
	PEAK_FLAG=False
	call_flag=True
