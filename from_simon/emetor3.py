...
def  chirpDetection(chirp_data_int,norm_chirp): 
	...
	while True:
		if sema>2*len(chirp_data_int):
			...
			print max(conv_env)
			if  max(conv_env)>0.6:  #freq_test  orand (sum(conv_env[peak_env-5:peak_env+5]>0.5*max(conv_env))<2)
				if peak_1==0:
...
if __name__=='__main__':
	...
	print "Nombre de frame de delay : "+str(chirpDetection(chirp_data_Int,norm_chirp)-44100) #88200 c'est le decalage inscrit dans receptor
	stream.stop_stream()
	stream.close()
	p.terminate()
